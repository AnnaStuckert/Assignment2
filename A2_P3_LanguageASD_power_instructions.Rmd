...
##Modelling
model <- lmer(CHI_MLU ~ Visit * Diagnosis * verbalIQ1 + (1 + Visit | Child.ID), df_train)
model1 <- lmer(CHI_MLU ~ Visit  + (1 + Visit | Child.ID), df, REML = F, control = lmerControl(optimizer = "nloptwrap", calc.derivs = F))
summary(model)
fixef(model)["Visit:DiagnosisTD:verbalIQ1"] <- 1

powerSim(model1, test = fixed("Visit"), nsim = 10)
```
