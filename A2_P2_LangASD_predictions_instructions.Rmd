...
prediction <- predict(m1, df_test[-2,], allow.new.levels = T)
rmse(actual, prediction)
...
mean(rmse0, na.rm = T) #0.3229 CHI_MLU ~ Visit + Diagnosis + (1+Visit|Child.ID)
mean(rmse1, na.rm = T) #0.3230 CHI_MLU ~ Visit * Diagnosis + (1+Visit|Child.ID)
#The best model seems to be the model where there is no interaction between Visit and Diagnosis, but i do not know why or how... Please help Kenneth or Riccardo
...
summary(m0)
berniemodel <- lmer(CHI_MLU ~ Visit * Diagnosis + (1+Visit|Child.ID), data=bernie, REML = F)
