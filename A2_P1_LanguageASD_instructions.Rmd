...
model1 <- lmer(CHI_MLU ~ VISIT * DIAGNOSIS + (1+VISIT|SUBJ), data = df, REML = F) #We expect that the development per visit is different and that each subject develops differently, hence the random slopes and intercepts.
summary(model1)
...
```{r ex3, include = FALSE}
model2 <- lmer(MOT_MLU ~ VISIT * DIAGNOSIS + (1+VISIT|SUBJ), data = df)
summary(model2)
...
```{r ex4, include = FALSE}
model3 <- lmer(CHI_MLU ~ VISIT * DIAGNOSIS * EXPRESSIVELANGRAW1 + (1 + VISIT | SUBJ), data = df2, REML = F)
summary(model3)

model4 <- lmer(CHI_MLU ~ VISIT * DIAGNOSIS * MULLENRAW1 + (1 + VISIT | SUBJ), data = df2, REML = F)

model5 <- lmer(CHI_MLU ~ VISIT * DIAGNOSIS * SOCIALIZATION1 + (1 + VISIT | SUBJ), data = df2, REML = F)

model6 <- lmer(CHI_MLU ~ VISIT * DIAGNOSIS * ADOS1 + (1 + VISIT | SUBJ), data = df2, REML = F)

model7 <- lmer(CHI_MLU ~ VISIT * DIAGNOSIS * GENDER + (1 + VISIT | SUBJ), data = df2, REML = F)
```
